...
private func exerciseDurabilityJourney() async throws {
    ...
    for step in 1...(horizon * SharedRules.inSeasonWeeks) {
        var advanced = false
        for attempt in 0..<4 {
            // These are the same app-layer entry points the root calls. Preparation and delegation
            // are harmless when their board is already settled; retrying is what clears an
            // offseason decision that appears at a season boundary without calling the engine.
            ...
            if attempt == 3 {
                throw JourneyError(
                    "app-layer advance stalled at \(expected): \(store.statusMessage ?? "no refusal")"
                )
            }
        }
