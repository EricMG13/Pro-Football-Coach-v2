...
These targets remain part of the product direction. Machine-measured targets become release claims
only after their dedicated instruments pass on the required hardware and run in CI; player-observed
targets are advisory and are not required for release approval:

| Target | Current evidence status |
|---|---|
| A season is completable in 6–8 hours | Non-gating product target; release approval does not require player timing observation |
