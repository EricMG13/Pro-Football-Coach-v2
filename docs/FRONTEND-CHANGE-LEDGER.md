...
| E18 | **TODO** | **At AX5 the chrome bar reflows to one column and consumes roughly half the viewport**, pushing the surface title nearly off screen. It satisfies `04` section 7 — order preserved, nothing dropped — but a navigation band that leaves no room for the surface is a fault the drawings never faced. Decide before 2B draws its first surface: collapse to a disclosure at accessibility sizes, or scroll the band. | `Sources/ProFootballCoachUI/ForgeFieldChromeBar.swift`, `04` section 6.1f |
