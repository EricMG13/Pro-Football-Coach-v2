...
| E40 | **DONE** | Game detail / box score drawn as a Dossier READOUT with the 271-of-852 staged column and vertical hard seam, 14 distinct retained-fact roles under the 72-point ceiling, zero gold, zero embers, no ghost, one `ground1` background, and inert dense rows only. The authoritative close callback remains a quiet 44 pt navigation control. **Adaptation-rule handling:** generated team names may scale only to 0.8 at standard size; at AX5 every one-line cap lifts and the whole surface becomes one outer scroll column, so no retained evidence is dropped or clipped. The standard render found the chrome at `0,0` instead of its authored `10,8`; the focused contract now pins that origin. A real AX5 XCTest swipe reached the final injury evidence at 844×390. XCTest serializes the attachment in the sensor's portrait pixel orientation, so the retained raw capture is losslessly normalized by 270° for review; that serialization is not a production-layout defect. | `Sources/ProFootballCoachUI/GameDetailBoxScoreView.swift`, `Sources/ProFootballCoachUI/RootView.swift`, `Tests/SimTests/Suites/DesignContractTests.swift`, `Tests/ProFootballCoachUITests/ProFootballCoachUITests.swift`, E18 |
