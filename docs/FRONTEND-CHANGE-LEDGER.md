...
| E39 | **DONE** | **Two Aftermath defects found by rendering, not by a binding test.** In the fixed standard flood, the FINAL/venue/result row competed with the oversized score, headline and flexible action row, so SwiftUI compressed the small outcome row out of the visible strip. It is now a vertically fixed root overlay above that flexible content; no new fact or gold spend was added. The standard plan panel already used an internal scroll for retained call-ins and injuries, but gave no indication that the lower groups were below its viewport. It now states `Scroll for call-ins and injuries` only at standard Dynamic Type when either retained group exists; AX5 has the outer accessible scroll instead. A real AX5 swipe reaches the same outcome content, so E18 remains a global-chrome degradation rather than an Aftermath-local redesign. | `Sources/ProFootballCoachUI/AftermathView.swift`, `Tests/ProFootballCoachUITests/ProFootballCoachUITests.swift`, E18 |
