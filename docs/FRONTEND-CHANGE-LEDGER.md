...
| E39 | **DONE** | **Two Aftermath defects found by rendering, not by a binding test.** In the fixed standard flood, the FINAL/venue/result row competed with the oversized score, headline and flexible action row, so SwiftUI compressed the small outcome row out of the visible strip. It is now a vertically fixed root overlay above that flexible content; no new fact or gold spend was added. The authored standard studied region is only about 93 pt before its plan heading, so even the smallest nonempty retained Called in and Injuries groups exceed its remaining viewport: the plan therefore always states `Scroll for call-ins and injuries` and owns an internal scroll at standard Dynamic Type. Its runtime proof starts with the one-line-per-group minimum fixture, confirms the final injury is hidden, swipes that internal scroll, then confirms it is reachable. AX5 has the outer accessible scroll and no nested-scroll cue. A real AX5 swipe reaches the same outcome content, so E18 remains a global-chrome degradation rather than an Aftermath-local redesign. | `Sources/ProFootballCoachUI/AftermathView.swift`, `Tests/ProFootballCoachUITests/ProFootballCoachUITests.swift`, E18 |
