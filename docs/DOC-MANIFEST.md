...
## 1. How to read the four classifications
...
| `SUPERSEDED-BY <reference>` | An **external** reference — a design-tool project, not a path in this tree — has been replaced by another named here. The old reference still exists and is still readable where it lives; it has lost its authority, not its content. | Where it always lived. Not in this repository's history. |
...
| **Press Box**, Claude Design project `3e8bedda-4c56-4be1-8f3a-98f9c2e82d9d` | `SUPERSEDED-BY Forge Field` | Owner-approved 2026-08-23, superseded 2026-08-29 by Forge Field. Its `AUTHORITY.md` boundary section carries forward verbatim — Forge Field inherits the same limits and overrides no fact, no read model, no legal guardrail and no accessibility floor. | Composition reference and change history. `docs/FRONTEND-CHANGE-LEDGER.md` stays live; see its Part E. |
...
| `docs/proofs/figma-pool-2026-08-13/`, `docs/proofs/stitch-2026-08-13/` | Dated proof artefacts beneath the already-listed `docs/proofs/` | **DELETED 2026-09-01.** Historical prototype captures had no authority and are recoverable from Git history; the retained proof package is listed above |
...
| `docs/handoff/` | Dated session handoffs: what a tranche finished, what it left open, and which verification a successor still owes | **None.** A record of one session's state on one date, exactly as `docs/plans/`. Where a handoff disagrees with canon or with the tree, the handoff is the stale one |
