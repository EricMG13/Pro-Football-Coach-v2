...
public struct CareerArcState: Codable, Sendable, Equatable {
    ...
    /// `02` section 7: the four dispositions belong to a club, so a coach arriving from out of work
    /// starts level with them. A promotion is the exception and carries its support -- see
    /// `acceptOpportunity`, which is the only appointment path that can be reached while employed.
    ///
    /// Never called by `establishCollegeJob`. That method looks like the arrival and is not:
    /// `prepareSeasonExpectation` calls it every week to rebuild a missing job record from the
    /// seated control, so resetting inside it wipes support continuously and keeps a coach employed
    ...
    public mutating func acceptOpportunity(
        ...
        // `02` section 7: losing a job resets support, earning one carries it. The distinction is
        // what the coach was doing the moment before -- a promotion out of a job they were
        // succeeding in is hired *because* of that standing, and resetting would strip what earned
        // it. Only an arrival from out of work starts level.
        if status != .employed {
